...
from chess_graphics_functions import *

#To run game cd to v5 folder & run python3 chess_graphics.py
...
s = 'sounds'
movement_sound = pygame.mixer.Sound(os.path.join(s, 'move_sound.mp3'))
board_start_sound = pygame.mixer.Sound(os.path.join(s, 'board_start.mp3'))
...
#Chess Board
board = pygame.image.load(r'images/chess_img.jpeg')
...
piece_rect = [[' ' for i in range(8)] for i in range(8)]
...
while not game_over: #while game still running
    for event in pygame.event.get(): #Checks for any event that occurs in pygame
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if start_button_active == True:
                ...
                error_message_string = "%s please enter a move" % starter.name
                set_starting_board()
                pygame.mixer.Sound.play(board_start_sound) #Trigger piece moved sound
                start_button_active = False
            
            #####try obtain piece information for clicked element ##remove for this version
            #for x in range(0,len(Board.board[0])):
            #    for y in range(0,len(Board.board[0])):
            #        piece_rect[x][y] = Board.board[0][0].image.get_rect()
            
           # print(piece_rect[0][0])
           # if piece_rect[0][0].collidepoint(event.pos):
            #    print("Clicked")
            ...
            if input_move_rect_active == True:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if validate_input(input_move) == True:
                        ...
                        if(validate_player_piece_selection2(input_move,current_player) == True):
                            #if valid selection, execute piece function
                            #move_piece(input_move,current_player) #Returns true/false if move was executed
                            if (executePieceFunction(input_move,current_player) == True):
                                pygame.mixer.Sound.play(movement_sound) #Trigger piece moved sound
                                current_player = changePlayer(current_player,player1,player2)
                                error_message_string = "%s please enter a move" % current_player.name

                            else:
                                error_message_string = executePieceFunction(input_move,current_player)
                                
                        else:
                            error_message_string = validate_player_piece_selection2(input_move,current_player)
    ...
    if welcome_screen == True:
        ...
        print_piece_graphics(screen)
        black_pieces,white_pieces = check_piece_count()
        check_piece_loss(black_pieces,white_pieces,screen,header_font)
